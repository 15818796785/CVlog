...
for x_list in tqdm.tqdm(X, desc='adding masks'):
    temp_X_blured = []
    for x in x_list:
        blured_face = preprocessing_utils.random_rotate(x)
...
for i, subject_images in enumerate(X_RandomRotate):
...
for i, subject_images in enumerate(X_masked_RandomRotate):

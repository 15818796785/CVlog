...
X_GaussianBlur = []
for x_list in tqdm.tqdm(X, desc='gaussian blur'):
    temp_X_blured = []
    for x in x_list:
        blured_face = preprocessing_utils.apply_gaussian_blur(x)
        temp_X_blured.append(blured_face)
    X_GaussianBlur.append(temp_X_blured)

X_masked_GaussianBlur = []
for x_list in tqdm.tqdm(X_masked, desc='gaussian blur'):
    temp_X_blured = []
    for x in x_list:
        blured_face = preprocessing_utils.apply_gaussian_blur(x)
        temp_X_blured.append(blured_face)
    X_masked_GaussianBlur.append(temp_X_blured)

X_GaussianBlur_dataset_path = '../GeorgiaTechFaces/Gaussianprocessedset_1'
for i, subject_images in enumerate(X_GaussianBlur):
    subject_folder = os.path.join(X_GaussianBlur_dataset_path, f"s{str(i + 1).zfill(2)}")
    os.makedirs(subject_folder, exist_ok=True)
    for j, img in enumerate(subject_images):
        cv2.imwrite(os.path.join(subject_folder, f"{str(j + 1).zfill(2)}.jpg"), img)

X_masked_GaussianBlur_dataset_path = '../GeorgiaTechFaces/GaussianMaskprocessedset_1'
for i, subject_images in enumerate(X_masked_GaussianBlur):
    subject_folder = os.path.join(X_masked_GaussianBlur_dataset_path, f"s{str(i + 1).zfill(2)}")
    os.makedirs(subject_folder, exist_ok=True)
    for j, img in enumerate(subject_images):
        cv2.imwrite(os.path.join(subject_folder, f"{str(j + 1).zfill(2)}.jpg"), img)
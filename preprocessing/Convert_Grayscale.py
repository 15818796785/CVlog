...
for i, subject_images in enumerate(X_ConvertGrayscale):
    subject_folder = os.path.join(X_ConvertGrayscale_dataset_path, f"s{str(i + 1).zfill(2)}")
    os.makedirs(subject_folder, exist_ok=True)
    for j, img in enumerate(subject_images):
        cv2.imwrite(os.path.join(subject_folder, f"{str(j + 1).zfill(2)}.jpg"), img)

X_masked_ConvertGrayscale_dataset_path = '../GeorgiaTechFaces/ConvertGrayscaleMaskprocessedset_1'
for i, subject_images in enumerate(X_masked_ConvertGrayscale):
    subject_folder = os.path.join(X_masked_ConvertGrayscale_dataset_path, f"s{str(i + 1).zfill(2)}")
    os.makedirs(subject_folder, exist_ok=True)
    for j, img in enumerate(subject_images):
        cv2.imwrite(os.path.join(subject_folder, f"{str(j + 1).zfill(2)}.jpg"), img)